...
### 2. Первый запуск (рекомендуется) 🚀

Для быстрой настройки и запуска используйте интерактивный скрипт:
...
Скрипт поможет:
- ✅ Выбрать LLM модель (от легкой 3B до мощной 14B)
- ✅ Создать файл `.env` с настройками
- ✅ Загрузить выбранную модель в Ollama
- ✅ Собрать и запустить все сервисы

**Доступные модели:**
| Модель | VRAM | Особенности |
|--------|------|-------------|
| `qwen2.5:14b-instruct` | ~8GB | Рекомендуется, лучший русский язык |
| `qwen2.5:7b-instruct` | ~4GB | Легкая, быстрая |
| `mistral-nemo:12b` | ~7GB | Хороший баланс |
| `llama3.2:3b` | ~2GB | Минимальная |

---

### 2a. Ручная настройка (альтернатива)

<details>
<summary>Нажмите для раскрытия ручной настройки</summary>
...
```bash
cp .env.example .env
# Отредактируйте .env, выберите модель в OLLAMA_MODEL
```
...
# Посмотреть логи Ollama (модель загружается автоматически)
docker-compose logs -f ollama
```

**Примечание:** При первом запуске Ollama автоматически загрузит модель из `.env` в память. Это займёт 30-60 секунд. Смотрите логи выше.
